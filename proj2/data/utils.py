...
def extract_train_valid(allx, ally, samples_class, valid_size):
    samples_extracted = defaultdict(lambda: 0)
    idx_train, idx_valid = [], []
    for i, y_vec in enumerate(ally):
        y = np.argmax(y_vec)
        if samples_extracted[y] < samples_class:
            idx_train.append(i)
            samples_extracted[y]+= 1
        elif len(idx_valid) < valid_size:
            idx_valid.append(i)
...
def load_data(dataset_str, samples_class=200):
    ...
    _, _, tx, ty, allx, ally, graph = tuple(objects)
    ...
    idx_train, idx_val = extract_train_valid(allx, ally, samples_class, 500)
...
if __name__ == "__main__":
    adj, _, y_train, _, _, _, _, _ = load_data("citeseer", samples_class=1000)
    plot_subgraph(adj, np.argmax(y_train, axis=1))
