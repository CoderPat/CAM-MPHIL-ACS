...
def scatter_update_tensor(tensor, indices, updates):
    ...
    masked_tensor = tensor * tf.cast(tf.expand_dims(mask,axis=1), tf.float32)
    new_entries = tf.scatter_nd(tf.expand_dims(indices, axis=1), updates, shape=tf.shape(tensor))
    return masked_tensor + new_entries


class Seqgraph2Seq(BaseEmbeddingsGNN):
    """
    """
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.placeholders['path_indexes'] = tf.placeholder(tf.int32, [None, self.max_path_size], name='path_indexes')
        self.placeholders['path_lens'] = tf.placeholder(tf.int32, [None], name='path_lens')
    ...
    def compute_final_node_representations(self) -> tf.Tensor:
        ...
        primary_path = tf.nn.embedding_lookup(pad_embbeddings, self.placeholders['path_indexes'])

        path_representation = self.build_path_rnn(primary_path, self.placeholders['path_lens'])

        flat_representation = tf.reshape(path_representation, (num_graphs * self.max_path_size, self.params['hidden_size']))
    ...
    def process_data(self, data, mode):
        ...
        if mode == ModeKeys.TRAIN and (self.params['input_shape'] is None or self.params['target_vocab_size'] is None):
            ...
            for g in data:
                ...
                self.max_path_size = max(self.max_path_size, len(g['primary_path']))
    ...
    def make_minibatch_iterator(self, data: Any, mode):
        ...
        while num_graphs < len(data):
            ...
            while num_graphs < len(data) and node_offset + final_graph_nodes < self.params['batch_size']:
                ...
                pad_len = self.max_path_size - len(cur_graph['primary_path'])
                batch_path_lens.append(len(cur_graph['primary_path']))
                batch_path_indexes.append(np.concatenate((cur_graph['primary_path'] + node_offset + 1,
                                                          pad_len * [0])))
            ...
            batch_feed_dict = {
                ...
                self.placeholders['path_indexes'] : np.array(batch_path_indexes),
                self.placeholders['path_lens'] : np.array(batch_path_lens)
            }
            if self.params['attention'] is None:
